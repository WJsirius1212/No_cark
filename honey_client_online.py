...
#
    ...
    masterkey = request.json['masterkey']
    domain_pw_map = dict()
    domain_pw_map[get_exact_domain(domain)]=password

    # print(masterkey)
    hv = HoneyVault(VAULT_FILE, masterkey)
    hv.add_password(domain_pw_map)
    print(hv.get_sample_decoding())
    if request.json['confirm']==0:
        return ' '.join(hv.get_sample_decoding())
    elif request.json['confirm']==1:
        hv.save()
        return 'ok'
    # print(request.json['confirm'])
    # return 'ok'
...
def get_pass():
    ...
    return hv.get_password(get_exact_domains((domain,)))
...
def default(*args):
    ...
    print('\n'.join("%s : %s" % (k, v().split('\n')[1])
                    for k, v in list(command_func_map.items())
                    if k != '-help'))
